...
class MonitorJuSuporte extends Command
{
    /**
    ...
    {
        $url = 'https://suporte.janelaunica.com.br/home';
        $timeout = 5;
        $iterations = 6;
        $secondsBetween = 10;
        ...
        for ($i = 0; $i < $iterations; $i++) {
            try {
                ...
                if ($response->failed()) {
                    $publicMessage = "JU Suporte está indisponível (HTTP {$response->status()}). ";
                    $this->handleFailure("JU Suporte is down (HTTP {$response->status()})", $publicMessage);
                } else {
                    $publicMessage = "JU Suporte está operando normalmente.";
                    $this->handleSuccess("JU Suporte is UP (HTTP {$response->status()})", $publicMessage);
                }
            } catch (\Exception $e) {
                $publicMessage = "JU Suporte está inacessível (timeout/erro de conexão). ";
                $this->handleFailure("JU Suporte is unreachable (Timeout/Error: {$e->getMessage()})", $publicMessage);
            }
    ...
    private function handleFailure(string $consoleMessage, string $publicMessage)
    {
        ...
        $incidentName = 'Incidente: JU Suporte';
